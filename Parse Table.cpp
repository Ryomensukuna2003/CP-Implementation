...
void solve() {
    int K, x;
    cin >> K >> x;
    vector<vector<long long>> st(K + 1, vector<long long>(K));
    vll vec1;
    forin(K, vec1);
    ...
    for (int i = 1; i <= log2(K); i++) {
        for (int j = 0; j + (1 << i) <= K; j++) {
            st[i][j] = min(st[i - 1][j], st[i - 1][j + (1 << (i - 1))]); // <-- Change the min to any function you like i.e., max,gcd,min
        }
    }
    ...
    for (int i = 0; i < x; i++) {
        ...
        int z = log2(r - l + 1);
        int minimum = min(st[z][l], st[z][r - (1 << z) + 1]);
        cout << minimum << endl;
    }
}
